...
// функция проверяет наличие пустого места в массиве перед занятым блоком
int IsBeforeFree(BlockMeta** freeMem, BlockMeta* block)
{
    BlockMeta *tmp = *freeMem;
    int ThereIsBlockBefore = 0;
    ...
    return ThereIsBlockBefore;
}

void Optimisation(BlockMeta **p_freeMem, BlockMeta* block)
{
    BlockMeta *tmp = *p_freeMem;
    
    while (tmp != NULL)
    {
        if (IsBeforeFree(p_freeMem, block))
        {
